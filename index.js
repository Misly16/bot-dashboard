...
var express = require("express"), session = require("express-session"),
    ...
    Strategy = require("./passport/index.js").Strategy, app = express();
...
app.get("/login",
        ...
        function(req, res) {});
app.get(
    "/callback", passport.authenticate("discord", {failureRedirect : "/login"}),
    function(req, res) { res.redirect("/dashboard"); } // auth success
);
...
app.get("/user", checkAuth, function(req, res) { res.json(req.user); });
...
app.get("/dashboard/server/:id", checkAuth, async function(req, res, next) {
  ...
  let username = req.user
  let serverinfo = req.user.guilds.filter((e) => e.id == req.params.id &&
                                                 e.permissions & 0x8)
...
// CONFIG PAGES
app.get("/dashboard/server/:id/config/welcome", checkAuth,
